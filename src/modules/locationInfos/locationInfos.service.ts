...
export class LocationInfosService {
   ...
   * @param location 
   */
  async getLocationInfo(locationQuery: LocationInfoQuery): Promise<LocationInfo> {
    ...
    if (locationQuery.country)
      query.andWhere("LocationInfo.city = :city", { country: locationQuery.country });
    if (locationQuery.zip)
      query.andWhere("LocationInfo.zip = :zip", { zip: locationQuery.city });
    if (locationQuery.location) {
      query.andWhere("LocationInfo.latitude = :latitude", { latitude: locationQuery.location.coordinates[0] });
      query.andWhere("LocationInfo.longitude = :longitude", { longitude: locationQuery.location.coordinates[1] });
    }
